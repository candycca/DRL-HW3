...
class DQNReplayTrainer:
    def __init__(self, config: TrainConfig, seed: int, mode: str, device: str = "cpu"):
        ...
        self.target_net = QNetwork(state_dim, len(ACTIONS), config.hidden_dim).to(device)
    ...
    def train(self, on_episode_end: Optional[Callable[[int, "DQNReplayTrainer"], None]] = None) -> List[Dict[str, float]]:
        metrics = []
        step_count = 0
