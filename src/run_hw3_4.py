...
from rainbow_dqn import RainbowDQNTrainer
...
EXPERIMENTS = {
    "baseline_random": {"trainer": DQNReplayTrainer, "mode": "random"},
    "rainbow_random": {"trainer": RainbowDQNTrainer, "mode": "random"},
}
...
def main():
    base_output = Path("outputs/hw3-4")
    ...
    plot_loss_compare(compare_data, str(summary_dir / "loss_compare_hw3-4.png"))
    metrics_table = metrics_to_table(summary_rows)
    metrics_table.to_csv(summary_dir / "metrics_table_hw3-4.csv", index=False)

    with (summary_dir / "config_hw3-4.json").open("w") as f:
