...
from dueling_dqn import DuelingDQNTrainer
from dueling_double_dqn import DuelingDoubleDQNTrainer
...
EXPERIMENTS = {
    "baseline_player": {"trainer": DQNReplayTrainer, "mode": "player"},
    "double_player": {"trainer": DoubleDQNTrainer, "mode": "player"},
    "dueling_player": {"trainer": DuelingDQNTrainer, "mode": "player"},
    "dueling_double_player": {"trainer": DuelingDoubleDQNTrainer, "mode": "player"},
}
...
def main():
    base_output = Path("outputs/hw3-2")
    ...
    plot_loss_compare(compare_data, str(summary_dir / "loss_compare_hw3-2.png"))
    metrics_table = metrics_to_table(summary_rows)
    metrics_table.to_csv(summary_dir / "metrics_table_hw3-2.csv", index=False)

    with (summary_dir / "config_hw3-2.json").open("w") as f:
